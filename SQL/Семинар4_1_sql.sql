DROP DATABASE IF EXISTS lesson_4_1;
CREATE DATABASE lesson_4_1;
USE lesson_4_1;
-- Таблицы 
-- 1. пользователи
...
SELECT
	u.id,
	u.firstname, -- users = u, имя
    u.lastname, -- users = u, фамилия 
...
SELECT
    ...
    u.id AS users_id
FROM messages ms
RIGHT JOIN users u ON ms.from_user_id = u.id
WHERE ms.from_user_id IS NULL;

SELECT * FROM friend_requests;

SELECT
	COUNT(fr.status) AS 'Кол.во друзей',
	/*fr.initiator_user_id,
    fr.target_user_id,
    fr.status,*/
    u.id AS users_id
FROM users u 
LEFT JOIN friend_requests fr ON (fr.initiator_user_id = u.id OR fr.target_user_id = u.id)  
...
SELECT
	DENSE_RANK () OVER(PARTITION BY users_firstname ORDER BY m.id) AS dense_rank_users,
	users_firstname AS users_firstname,
    users_lastname AS users_lastname,
	MAX(`dense_rank`) 
FROM(
SELECT
	u.id AS users_id,
    ...
    m.id AS messages_id,
    m.from_user_id AS messages_from_user_id,
    m.to_user_id AS messages_to_user_id,
    -- DENSE_RANK () OVER(ORDER BY u.id) AS `dense_rank_users`,
	DENSE_RANK () OVER(PARTITION BY u.id ORDER BY m.id) AS `dense_rank`
FROM users u
LEFT JOIN messages m ON u.id = m.from_user_id) AS dense_rank_messages
GROUP BY users_firstname, users_lastname
ORDER BY MAX(`dense_rank`) DESC;
...
SELECT
    ...
    RANK() OVER (ORDER BY COUNT(m.id) DESC) AS message_rank
FROM users u
LEFT JOIN messages m ON u.id = m.from_user_id
...
SELECT 
    ...
    to_user_id,
    created_at,
    LEAD(created_at) OVER (PARTITION BY from_user_id, to_user_id ORDER BY created_at) AS next_time,
...
SELECT
	m.from_user_id,
    CONCAT(u.firstname, ' ', u.lastname) AS 'From who',
    COUNT(*) AS cnt
FROM messages m
...
OR
(fr.target_user_id = m.to_user_id AND fr.initiator_user_id  = m.from_user_id)
