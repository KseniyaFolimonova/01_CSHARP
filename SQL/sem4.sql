...
SELECT
	t.surname, -- teacher = t
    l.course -- lesson = l
FROM teacher t -- имя_таблицы псевдоним, левая таблица
LEFT JOIN lesson l -- LEFT OUTER JOIN = LEFT JOIN (сокращенная форма), правая 
ON t.id = l.teacher_id;
...
SELECT
	t.surname, -- teacher = t
    l.course -- lesson = l
FROM lesson l -- имя_таблицы псевдоним, левая таблица
RIGHT JOIN teacher t -- LEFT OUTER JOIN = LEFT JOIN (сокращенная форма), правая 
ON t.id = l.teacher_id
WHERE l.teacher_id IS NULL; -- teacher_id - это NULL? Выводим строчки с NULL
